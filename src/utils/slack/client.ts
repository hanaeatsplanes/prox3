import type { ConversationRepliesResponse, SlackApiResponse } from "@/models/slack";
...
async function slackFetch(url: string, options: RequestInit, endpoint: string): Promise<Response> {
	let attempt = 0;
	const maxRetries = 3;
	...
	throw new Error(`[slack] ${endpoint} rate limit exceeded after ${maxRetries} retries`);
}

async function readSlackResponse(response: Response, operation: string) {
	if (response.status === 429) {
		const retryAfter = response.headers.get("Retry-After") || "60";
		throw new Error(`[slack] ${operation} rate limited, retry after ${retryAfter}s`);
	}
	...
	if (!data.ok) {
		throw new Error(`[slack] ${operation} error: ${data.error || "unknown_error"}`);
	}
...
export async function chatUpdate(ts: string, channel: string, content: string | object[]) {
	const isText = typeof content === "string";
	const response = await slackFetch(
...
export async function chatPostEphemeral(channel: string, user: string, text: string) {
	const response = await slackFetch(
